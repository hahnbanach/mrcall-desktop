...
- **2026-05-06 — `_store_message_from_event` JID extraction fix.** The neonize `MessageEv.Info` is a `MessageInfo` proto whose JID-bearing fields (`Chat`, `Sender`, `IsFromMe`, `IsGroup`) live on a nested `MessageSource`, not on `Info` directly; the push-name field is spelled `Pushname` (lowercase `n`), not `PushName`. The previous code read them straight off `info`, so every `hasattr` returned False and rows landed with empty `chat_jid` / `sender_jid` / `sender_name`. Symptom on `mario.alemi@gmail.com` (UID `HxiZh…`): 36 messages stored, all with empty `chat_jid` (server breakdown `{'(no jid)': 36}`); WhatsApp tab showed zero conversations. Fix: read `info.MessageSource.Chat` / `info.MessageSource.Sender` / `info.MessageSource.IsFromMe` / `info.MessageSource.IsGroup` and `info.Pushname`; format JIDs as `f"{jid.User}@{jid.Server}"` via the new `_format_jid` helper because `str(jid)` returns the protobuf debug repr (`'User: "..."\nServer: "..."\n'`), not a usable wire string. Tests: `engine/tests/whatsapp/test_sync.py` (5/5 green) — real `Neonize_pb2.Message` events stored via real `WhatsAppSyncService` against a per-test SQLite DB; one end-to-end case verifies the `whatsapp.list_threads` filter accepts the resulting `user@s.whatsapp.net` rows. **Live Mac verification still pending** (mandatory per the discipline rule — see "What Is In Progress").
- **2026-05-07 — LID contact resolution + group titles + jid-as-phone cleanup.**
  - WhatsApp's privacy mode delivers many chats and group senders as `<digits>@lid` pseudonyms instead of phone JIDs. The numeric local-part is NOT a phone number — rendering it as `+<digits>` produced bogus US/intl-looking numbers under chats whose contact was actually Italian. Same shape on group threads: the bare jid (`393317059612-1507147748`) was being rendered as `+393317059612-1507147748` underneath the group title.
  - Fix in `zylch/rpc/whatsapp_actions.py:whatsapp_list_threads`: skip the phone-from-jid fallback whenever `is_group` or `jid.endswith("@lid")` — leave `phone=None` so the renderer falls back cleanly. Also stop using the latest message's `sender_name` as a 1-on-1 chat label when that message is `is_from_me=True` (it's the user's own name, not the peer's); track the most recent incoming sender_name per chat instead.
  - Fix in `zylch/whatsapp/sync.py:_jid_to_phone`: only `@s.whatsapp.net` produces a phone number; `@lid` and `@g.us` return `""`. Old rows with bogus `phone_number='+<lid>'` cleared via SQL on the gmail profile (2 rows).
  - **LID → real contact resolution, fully local.** New `WhatsAppSyncService.sync_lid_contacts()` reads neonize's own session DB (`~/.zylch/profiles/<uid>/whatsapp.db`) which whatsmeow populates from the user's address book during normal app-state sync. Two tables: `whatsmeow_lid_map(lid, pn)` for the LID↔phone mapping and `whatsmeow_contacts(their_jid, full_name, first_name, push_name, business_name)` for display names. The method opens the file read-only (WAL is concurrency-safe), looks up each LID seen in our `whatsapp_messages`, fetches both the LID row and the resolved `<pn>@s.whatsapp.net` row, and picks the best name with priority `full_name → business_name → first_name → push_name`. **No StarChat round-trip, no neonize RPC needed** — all the data is already on disk.
  - Wired into `whatsapp.connect` right after `sync_groups`. On the gmail profile: 50 LIDs resolved on first run, including the previously-broken thread (`19095575629933@lid` → "Alessandro Simonetti", `+393395040816`). Future LIDs picked up automatically each time the user clicks Connect.
  - **Live-verified on Mario's Mac (commit `9eee73c2`).** Group titles real, 1-on-1 LID chats resolve to real contact names, no `+<lid>` subtitles.
- **2026-05-07 — auto-reconnect at sidecar boot + status semantics + connect supersede.**
  - `rpc/server.py:_auto_reconnect_whatsapp()` runs in the background on every sidecar start. If `whatsapp.db` exists on disk it fires `whatsapp.connect` so MessageEv flow without a manual Reconnect click. Skips silently if no session is paired. Live-verified: cold-start of the app reconnects within ~20s, sync_contacts/sync_groups/sync_lid_contacts run automatically.
  - `whatsapp.status` now returns `connected = is_connected AND is_logged_in`. Previously it returned True during the QR-emit phase (socket up but session expired) — the renderer drew the chat list as if we were live even though no MessageEv would ever come. This is the load-bearing fix that lets the renderer's privacy gate work.
  - `whatsapp.connect` now **cancels** a prior in-flight connect (typically the boot-time auto-reconnect stuck in QR limbo) instead of returning `"connect already in flight"`. A user clicking Reconnect gets a fresh attempt that emits a fresh QR. Best-effort `asyncio.wait_for(... timeout=2.0)` for graceful shutdown of the previous task; falls through if it doesn't yield.
- **2026-05-07 — message-row backfill on second arrival.**
  - `WhatsAppSyncService._upsert_message`: an existing row with empty `text` (placeholder from history sync) used to be skipped. Now if a second event arrives with a real `text` / `timestamp` / `sender_name` / `chat_jid` / `sender_jid`, we backfill those columns. Closes the long-standing `[empty]` rows that came from history-sync placeholders never being overwritten.
  - 30 fossil `[empty]` rows from before the placeholder typing fix were deleted manually on Mario's profile (DB-of-test, authorised). New empties won't accumulate now.
