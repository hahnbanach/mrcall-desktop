...
async def _run_memory(owner_id: str, store) -> tuple[int, int]:
    ...
    # Consolidation: retention, then duplicate entities in the company store,
    # deciding pairs only when the canary above left merging enabled. Runs only
    # when the store changed since the last sweep started (a join, a merge,
    # entities just written above), once per company — the other daemons on
    # the store see "another engine is sweeping". Same tolerance contract as
    # the task sweeps: a failure is logged, never raised.
    try:
        from zylch.memory.consolidation import consolidate, summary_lines

        sweep = await consolidate(owner_id, merge_enabled=worker.merge_enabled)
        for line in [] if sweep.get("skipped") else summary_lines(sweep):
            console.print(f"  [dim]memory: {line}[/dim]")
    except Exception as e:
        logger.warning(f"[memory] post-update consolidation failed: {e}")
