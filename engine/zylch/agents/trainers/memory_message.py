"""Message Memory Agent Builder — generates a personalised entity-extraction
prompt from the user's communication history (whatsapp-pipeline-parity Phase 2b).

Was ``memory_email.py`` until 2026-05-08. The trainer is still trained on
EMAIL history (richer signal for "who is this user, what do they do, who
they ignore") but the GENERATED prompt is channel-aware: at extraction
time the worker can hand it either an email envelope (process_email) or
a WhatsApp envelope (process_whatsapp_message). Same #IDENTIFIERS /
#ABOUT / #HISTORY output, same cross-channel ``person_identifiers`` index
catches the merge — provided the prompt emits ``Phone:`` in #IDENTIFIERS
for WhatsApp messages too.

The legacy module name and class name remain re-exported from
...
# Meta-prompt used to generate the message-memory agent. The generated
# prompt is channel-aware: it must extract entities from email envelopes
# AND from WhatsApp message envelopes the worker hands it. The
# `person_identifiers` index (Phase 1a) does the cross-channel merge,
...
- WhatsApp (1-on-1 messages with the user's contacts)
...
3. **EXTRACTION RULES**
   ...
   **CROSS-CHANNEL IDENTITY (CRITICAL):**
   The user may talk to the same contact via email AND via WhatsApp. To
   merge those two records into ONE blob, the #IDENTIFIERS block MUST
   carry every stable identifier you observe — especially:
   - `Email:` whenever an email address is present (signature, From,
     CC, body mention)
   - `Phone:` whenever a phone number is present (signature, From-name
     on a WhatsApp message, body mention) — write the number with
     country code when known (`+39 339 ...`)
   - `LID:` whenever a WhatsApp `<digits>@lid` pseudonym is present
     (this is NOT a phone number; do not invent a `+` prefix)

   Missing one of these for a WhatsApp blob is the difference between
   a single merged person and two duplicate blobs after the next email
   arrives.
...
The generated prompt will be used as a CACHED SYSTEM prompt. Each
extraction call passes a single user message of the form:
...
The envelope is one of two shapes — your prompt must handle both:

EMAIL envelope:
    From: sender@example.com
    To: recipient@example.com
    CC: (none) or comma-separated list
    Date: 2026-05-08T10:11:12+00:00
    Subject: <subject>
    Contact: sender@example.com  (the external party we're indexing)
...
threaded reply. Apply the SAME extraction logic in both cases. When the
WhatsApp envelope's `From:` line carries a phone, emit it as `Phone:` in
the resulting #IDENTIFIERS block — that is what lets the cross-channel
identity index find an existing email-derived blob for the same person.

OUTPUT ONLY THE PROMPT TEXT. No explanations, no markdown code blocks.
Just the prompt itself. Do NOT use Python `.format()` placeholders like
``{{from_email}}`` or ``{{body}}`` — the prompt is sent as a system
message, the per-call data arrives in the user message above."""


class MessageMemoryAgentTrainer:
    """Builds a personalised, channel-aware memory agent by analysing the
    user's email patterns. Trained on email (richer signal) but the
    generated prompt extracts entities from email AND WhatsApp envelopes.

    Was ``EmailMemoryAgentTrainer`` until 2026-05-08 (whatsapp-pipeline-parity
    Phase 2b). The legacy name is re-exported as an alias.
    """
...
"""
    ...
    async def build_memory_message_prompt(self) -> Tuple[str, Dict[str, Any]]:
        ...
        """
        logger.info(f"Building memory message prompt for {self.owner_id}")
...
EmailMemoryAgentTrainer = MessageMemoryAgentTrainer
EMAIL_AGENT_META_PROMPT = MESSAGE_AGENT_META_PROMPT


# Backward-compat method alias for callers that still invoke the old name.
async def _build_memory_email_prompt(self):  # noqa: D401
    return await self.build_memory_message_prompt()


MessageMemoryAgentTrainer.build_memory_email_prompt = _build_memory_email_prompt
