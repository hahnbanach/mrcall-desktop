...
EMAIL_AGENT_TOOLS = [
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "email_id": {"type": "string", "description": "The email UUID to fetch"}
            },
            "required": ["email_id"],
        },
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {"response": {"type": "string", "description": "Your response text"}},
            "required": ["response"],
        },
    },
    {
...
class EmailContextGatherer:
    ...
    def __init__(self, storage: Storage, search_engine: HybridSearchEngine, owner_id: str):
...
class EmailerAgent(SpecializedAgent):
    ...
    async def compose(
        ...
        context = await self.gatherer.gather(
            user_request=user_request, recipient_email=recipient_email, task_num=task_num
        )
...
Use the write_email tool to output your composed email."""
    ...
    async def run(
        ...
        context = await self.gatherer.gather(
            user_request=instructions, recipient_email=recipient_email, task_num=task_num
        )
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            prompt = self._build_run_prompt(
                instructions, context, accumulated_context, previous_draft
            )
            ...
            if tool_used == "search_memory":
                ...
                if not email_data.get("error"):
                    ...
                    logger.debug(
                        f"[EMAILER] Accumulated email: {email_data.get('subject', 'no subject')}"
                    )
        ...
        return {
            ...
            "tool_input": {},
            "result": {
                "response": "I gathered some information but could not complete the task. Please try being more specific about what you want me to do."
            },
        }
        ...
        if accumulated_context:
            ...
            for item in accumulated_context:
                if item["type"] == "search_memory":
                    ...
                    if emails:
                        context_text += (
                            f"**Email search for '{query}':** Found {len(emails)} emails:\n"
                        )
                        for e in emails:
...
use that information to write the email or provide the answer. Don't just report search results."""
    ...
    def _handle_tool_response(
        self, response, context: EmailContext, recipient_email: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        """
        result = {"tool_used": None, "tool_input": {}, "result": None}
            ...
            for block in response.content:
                if hasattr(block, "input"):  # ToolUseBlock
                    ...
                    if block.name == "write_email":
                        ...
                        result["result"] = {"response": block.input.get("response", "")}
    ...
    def _process_search_memory(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for r in results:
            formatted.append(
                {"content": r.content, "score": r.score if hasattr(r, "score") else None}
            )

        return {"results": formatted, "message": f"Found {len(results)} results for '{query}'"}
    ...
    def _process_search_emails(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            emails = self.storage.search_emails(owner_id=self.owner_id, query=query, limit=limit)
        except Exception as e:
        ...
        return {"emails": formatted, "message": f"Found {len(formatted)} emails for '{query}'"}
