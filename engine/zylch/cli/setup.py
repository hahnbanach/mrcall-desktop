...
def _prompt_with_existing(
    ...
    When ``allow_blank=True``, an empty input returns ``""`` instead of
    looping until the user provides a value (used for BYOK fields where
    blank means "use the default transport").
    """
...
def _run_wizard(env: dict, profile_name: str | None):
    ...
    click.echo("1. LLM credentials (optional)")
    click.echo(
        "   Zylch uses Claude. Provide an Anthropic API key to bring your "
        "own (BYOK), or leave blank to use MrCall credits (requires the "
        "desktop signin)."
    )
    click.echo()
    ...
    api_key = _prompt_with_existing(
        "Anthropic API key (blank = MrCall credits)",
        existing_key,
        hide=True,
        allow_blank=True,
    )
    ...
    logger.debug(
        f"[init] anthropic_api_key={'present' if api_key else 'absent'}",
    )
    ...
    # LLM — BYOK key is optional. When absent, the engine falls back
    # to MrCall credits (Firebase JWT). No SYSTEM_LLM_PROVIDER /
    # OPENAI_API_KEY anymore.
    if api_key:
        lines.append("")
        lines.append("# LLM (BYOK)")
    ...
    click.echo(
        "  LLM:    "
        + ("Anthropic (BYOK)" if api_key else "MrCall credits (Firebase signin)")
    )
    if email:
