...
class TaskWorker:
    ...
    async def _analyze_recent_whatsapp_events(self, concurrency: int = 5) -> None:
        ...
        Lives in its own method so the email contract stays untouched
        ...
        all_msgs = self.storage.get_unprocessed_whatsapp_messages_for_task(self.owner_id)
        if not all_msgs:
        ...
        def _mark_chat_processed(chat_jid: str) -> None:
            ...
            if ids:
                self.storage.mark_whatsapp_messages_task_processed(self.owner_id, ids)
        ...
        for chat_jid in list(user_replied.keys()):
            ...
            if reply_ts <= contact_ts:
                ...
                continue
            thread_tasks = self.storage.get_tasks_by_thread(
                self.owner_id, chat_jid, open_only=True
            )
            for t in thread_tasks:
                self.storage.complete_task_item(self.owner_id, t["id"])
                logger.info(
        ...
        async def _collect_wa(msg: Dict):
            ...
            phone, lid, sender_name = _resolve_wa_sender(msg, self.storage, self.owner_id)
            ...
            blob_context, blob_id = self._get_blob_for_contact(contact_key)
            thread_tasks = self.storage.get_tasks_by_thread(
                self.owner_id, chat_jid, open_only=True
            )
            existing_tasks_all: List[Dict] = list(thread_tasks)
            ...
            if phone:
                for ct in self.storage.get_tasks_by_contact_phone(self.owner_id, phone):
                    if ct["id"] not in existing_ids:
                        existing_tasks_all.append(ct)
                        existing_ids.add(ct["id"])
            ...
            try:
                topical_blob_ids: List[str] = list(
                    self.storage.get_blobs_for_whatsapp_message(self.owner_id, msg_id)
                    if msg_id
                    else []
                )
                if not topical_blob_ids and blob_id:
