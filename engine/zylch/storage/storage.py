...
class Storage:
    ...
    def get_open_tasks_by_blobs(
        ...
        with get_session() as session:
            ...
            # open). Widen `wanted` through the alias table both ways and
            # along chains, so a stale id and its keeper match each other.
            from zylch.memory.scope import resolve_aliases

            wanted = resolve_aliases(session, wanted)
            rows = (
