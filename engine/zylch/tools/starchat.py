...
class StarChatClient:
    ...
    async def initiate_outbound_call(
        ...
        """Submit an outbound call through the same API as the dashboard.

        Legacy overrides are rejected: the atom uses the saved caller assistant
        configuration, not per-call purpose/contact/caller variables.
        """
        from .mrcall.outbound import submit_outbound_call

        if caller_id or contact_id or variables:
            raise ValueError(
                "Outbound calls use the saved assistant configuration; overrides unsupported"
            )
        return await submit_outbound_call(self, phone_number, business_id)
