...
Historical snapshots; current state lives in active-context.md.
...
## 2026-09-11 — superseded app snapshot
...
| Date | What | Refs |
|---|---|---|
| 2026-06-16 | **SMS-sender Settings card** (`SMS_FROM`). `views/Settings.tsx` adds a field to read/write the per-business SMS sender via new `window.zylch.sms.getSender/setSender` (engine `rpc/sms_actions.py`, ≤11 chars, `auth_expired`-aware); a follow-up made the card load calmly when the initial `get` fails. Pairs with the engine `send_sms`-via-credits path (verified live: real SMS, 10 credits). | `dc92b2e` `c58d723` |
