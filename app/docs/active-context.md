...
| Date | What | Refs |
|---|---|---|
| 2026-05-31 | **Cross-machine transport, Phase 2 (app side).** `RpcClient` interface (`src/main/rpcClient.ts`) with two impls: `StdioRpcClient` (the renamed `SidecarClient` — local default, byte-identical) + `WebSocketRpcClient` (`src/main/wsRpcClient.ts`, `ws` npm). `spawnSidecar` is now a transport factory reading `src/main/backendConfig.ts` (`~/.zylch/backend-config.json`, default `{location:'local'}`). New out-of-band token path: `account:pushToken` IPC → main caches per window → WS handshake `Authorization: Bearer` (remote) / `account.set_firebase_token` forward (local); renderer `setTokenPusher` rewired to it. Settings `BackendLocationCard` (Local/Remote radio + URL + "Test connection" via `backend:testConnection`). `settings:getBackendLocation`/`setBackendLocation`. IdentityBanner shows `Remote: <url>`. WS "ready" synthesised on socket OPEN (engine emits no `engine.ready` over WS); `auth.refresh` ~30 min; reconnect on close 4401 w/ fresh token. | `docs/execution-plans/cross-machine-transport.md` Phase 2 |
