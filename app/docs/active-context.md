...
- Firebase config hard-coded in `firebase/config.ts` (project `talkmeapp-e696c`, same as the dashboard — single MrCall account spans both surfaces). **Persistence is `indexedDBLocalPersistence` (post-`aa75c6ec`)**: a signed-in user survives across app launches and FirebaseAuthGate enters the `binding` phase directly. UID-keyed profile binding means the restored session can only ever attach to its own profile dir. **Consequence:** IndexedDB is shared across all BrowserWindows of the app, so `auth.currentUser` is the same in every renderer — legacy windows (`?legacy=1`) must gate any UI that reads it on `isLegacyWindow()` from `lib/legacy.ts` (IdentityBanner in `App.tsx`, AccountCard + LLMProviderCard's `signedIn` flag in `Settings.tsx`). Without the gate, the legacy window advertises the identity of the proper-Firebase window and a Sign out from there clears the session globally.
