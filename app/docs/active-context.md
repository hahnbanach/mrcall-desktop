...
- `views/Settings.tsx` carries a new `LLMProviderCard` at the top of the LLM settings group: a radio toggle between **BYOK** (`anthropic` / `openai`) and **Use MrCall credits** (`mrcall`). The underlying setting is still `SYSTEM_LLM_PROVIDER` — the card just exposes a friendlier UX over a value that the schema-driven `<select>` below can also set. When `SYSTEM_LLM_PROVIDER` is missing from `.env` (the default for wizard-created profiles since 2026-05-04), the card derives the displayed mode from key presence: `ANTHROPIC_API_KEY` set → BYOK anthropic, `OPENAI_API_KEY` set → BYOK openai, otherwise → MrCall credits — matching the engine resolver.
