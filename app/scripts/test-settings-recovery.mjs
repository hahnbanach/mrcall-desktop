...
try{
 ...
 await page.goto(base+'/?state=byok')
 await page.getByText('BYOK (Anthropic key)',{exact:true}).waitFor()
 await page.getByRole('button',{name:'Switch to MrCall credits',exact:true}).click()
 await page.getByText(/Unsaved billing change: MrCall credits/).waitFor()
 assert.equal(await page.getByText('BYOK (Anthropic key)',{exact:true}).isVisible(),true)
 assert.equal(await page.evaluate(()=>window.fixture.writes()),0)
 assert.equal(await page.evaluate(()=>window.fixture.savedKey()),'<set>')
 assert.equal(await page.locator('#field-ANTHROPIC_API_KEY').count(),0)
 assert.equal(await page.getByRole('button',{name:'Top up credits',exact:true}).count(),0)
 await page.screenshot({path:join(output,'billing-unsaved.png'),fullPage:true})
 await page.getByRole('button',{name:'Save (1)',exact:true}).click()
 await page.waitForFunction(()=>window.fixture.writes()===1 && window.fixture.savedKey()==='')
 await page.getByRole('button',{name:'Top up credits',exact:true}).waitFor()
 assert.equal(await page.getByText(/Unsaved billing change:/).count(),0)
 assert.equal(await page.locator('#field-ANTHROPIC_API_KEY').count(),0)
 await page.getByRole('button',{name:'Use my own Anthropic API key (optional)',exact:true}).click()
 assert.equal(await page.locator('#field-ANTHROPIC_API_KEY').isVisible(),true)
 await page.getByRole('button',{name:'Hide optional API key',exact:true}).click()
 await page.getByRole('button',{name:'Top up credits',exact:true}).click()
 assert.deepEqual(await page.evaluate(()=>window.fixture.opened()),['https://dashboard.mrcall.ai/plan'])
 assert.equal(await page.evaluate(()=>window.fixture.writes()),1)
 await page.evaluate(()=>window.fixture.failTopup())
 await page.getByRole('button',{name:'Top up credits',exact:true}).click()
 await page.getByText('Could not open the top-up page. Please try again.',{exact:true}).waitFor()
 assert.equal(await page.evaluate(()=>window.fixture.writes()),1)
 await page.screenshot({path:join(output,'billing-saved-topup.png'),fullPage:true})
