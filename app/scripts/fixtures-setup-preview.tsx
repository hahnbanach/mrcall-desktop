...
const fake = {
  settings: {
    ...
    get: async () => { if (failed) throw Error('offline'); return {values: {EMAIL_ADDRESS: 'production@example.test', IMAP_HOST: 'imap.example.test', EMAIL_PASSWORD: '********', LLM_PROVIDER: 'mrcall'}} },
    testBackendConnection: async () => {calls.push('connection.proof'); return {ok: true, signedIn: true, uid: scenario === 'wrong-identity' ? 'another-profile' : 'fixture-profile'}},
  ...
  account: {whoAmI: async () => {if(failed) throw Error('offline'); return {signed_in: true, uid: scenario === 'wrong-identity' ? 'another-profile' : 'fixture-profile', email: 'production@example.test'}}},
...
Object.assign(window, {zylch: fake, setupFixture: {calls, activate: () => {provisionState = 'active'}}})
