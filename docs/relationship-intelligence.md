...
MrPark's goal is NOT to classify emails into categories.
MrPark's goal IS to identify what Mario needs to do.
...
MrPark now includes a **Relationship Intelligence** system that correlates communication across multiple channels (email, calendar, etc.) to identify relationship gaps and opportunities.

## Key Concept

**Instead of just managing individual channels separately, MrPark analyzes patterns across all communication to answer:**
...
### 1. CalendarSyncManager (`mrpark/tools/calendar_sync.py`)
...
### 2. RelationshipAnalyzer (`mrpark/tools/relationship_analyzer.py`)
...
0 5 * * * cd /Users/mal/starchat/mrpark && /usr/bin/python3 morning_sync.py >> logs/morning_sync.log 2>&1
```

### Option 2: Using CLI directly
```bash
# Run every day at 5am:
0 5 * * * cd /Users/mal/starchat/mrpark && echo "/sync\n/quit" | python -m mrpark.cli.main >> logs/morning_sync.log 2>&1
```
...
MrPark: [Drafts email based on meeting context]
```

## Personalized Filtering with Memory System

### Overview
The Relationship Analyzer integrates with MrPark's Memory System to learn your personal preferences for email filtering. This allows you to teach the system which emails to ignore or prioritize.
...
- **Email sync**: `mrpark/tools/email_sync.py`
- **Calendar sync**: `mrpark/tools/calendar_sync.py`
- **Relationship analyzer**: `mrpark/tools/relationship_analyzer.py`
- **Morning sync script**: `morning_sync.py`
- **Test script**: `test_relationship_intelligence.py`
- **CLI integration**: `mrpark/cli/main.py` (lines 1943-2078)
