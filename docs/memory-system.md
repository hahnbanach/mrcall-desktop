# MrPark Memory System

**Complete guide to MrPark's channel-based behavioral memory system**
...
MrPark implements a **ReasoningBank-inspired memory system** that learns from user corrections and applies them automatically in future interactions. The system is organized by **communication channels** (email, calendar, WhatsApp, phone calls, tasks) with separate memory for each channel.
...
**Human Analogy:** Humans have excellent behavioral memory. If someone corrects us once, we remember. MrPark should work the same way.

### The Solution: Channel-Based Memory

MrPark operates across 5 communication channels:
...
- ✅ Consistent with MrPark architecture (threads.json, tasks.json)
...
1. User asks MrPark to draft email → MrPark uses 'tu'
...
3. User adds memory:
   ...
   ```
4. Next time: MrPark automatically uses 'lei' because memory is injected into prompt
...
1. User creates meeting with international attendees → MrPark creates event without timezone
2. User corrects: "Add timezone - this is international!"
3. User adds memory:
   ```bash
   /memory --add "Missing timezone" "Always specify timezone (e.g., CET, PST) in event description" calendar
   ```
4. Next calendar event: MrPark automatically includes timezone
...
When MrPark performs a task, relevant memories are automatically injected:
...
### Implementation
- **Core class**: `mrpark/memory/reasoning_bank.py`
- **CLI integration**: `mrpark/cli/main.py`
- **Agent integration**: `mrpark/agent/core.py`
...
**Date:** 2025-11-19
**Rationale:** Consistent with MrPark architecture, human-readable
