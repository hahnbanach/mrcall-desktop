...
class RelationshipAnalyzer:
    ...
    def find_meeting_without_followup(
        ...
        gaps = []
        # Use naive UTC datetime for comparisons (consistent with email dates)
        ...
        for event in events.values():
            ...
            for attendee_email in event.get('external_attendees', []):
                # Look for emails with this contact after THIS meeting
                ...
                if not has_followup:
                    # Extract contact name from attendee list if available
                    contact_name = self._extract_name_from_attendees(
                        attendee_email,
                        event.get('attendees_with_status', [])
                    )

                    gaps.append({
                        "type": "meeting_no_followup",
                        "meeting_id": event['id'],
                        "meeting_summary": f"Meeting with {contact_name}",
                        "meeting_date": event['start'],
                        "contact_email": attendee_email,
                        "contact_name": contact_name,
                        "days_ago": (datetime.now(timezone.utc).replace(tzinfo=None) - start_dt).days,
                        "severity": "high" if (datetime.now(timezone.utc).replace(tzinfo=None) - start_dt).days > 2 else "medium",
    ...
    def find_email_tasks(
        ...
        for thread in threads.values():
            ...
            # Group by person (include ALL recent threads, not just those requiring action)
            if contact_email not in person_threads:
                person_threads[contact_email] = {
                    ...
                    'threads': [],
                    'has_open_thread': False  # Track if any thread requires action
                }

            person_threads[contact_email]['threads'].append(thread)

            # Track if this person has at least one open thread
            # Use 'open' field instead of 'requires_action' to respect manual closures
            if thread.get('open') and not thread.get('manually_closed'):
                person_threads[contact_email]['has_open_thread'] = True

        logger.info(f"Grouped into {len(person_threads)} people with active threads")

        # Analyze each person with Sonnet (but only if they have at least one open thread)
        tasks = []
        for person_data in person_threads.values():
            # Skip if no open threads (all conversations resolved)
            if not person_data.get('has_open_thread'):
                continue
    ...
    def find_silent_contacts(
        ...
        for contact in contact_interactions.values():
            ...
            if last_interaction and last_interaction < cutoff_date:
                days_silent = (datetime.now(timezone.utc).replace(tzinfo=None) - last_interaction).days
    ...
    def _analyze_person_for_task(self, person_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # Compute relationship signals BEFORE calling Claude
...
Anna Bianchi from Sesamers sends perfectly formatted email with bullet points,
...
RETURN ONLY THE JSON, NOTHING ELSE."""
    ...
    def _has_email_after(
        ...
        Args:
            ...
            after_date: Check for emails after this datetime
            window_hours: Window in hours (not strictly enforced - any email after counts)
            threads: Thread dictionary

        Returns:
            True if there's any email after the meeting
        """
        for thread in threads.values():
            ...
            # Check if email is after meeting
            try:
                # Parse RFC2822 format (e.g., "Thu, 20 Nov 2025 18:48:40 +0100")
                last_msg_date_str = thread.get('last_message_date', '')
                if not last_msg_date_str:
                    continue

                # Use parsedate_to_datetime for RFC2822 format, then strip timezone
                last_msg_date = parsedate_to_datetime(last_msg_date_str).replace(tzinfo=None)

                # Check if email is within the follow-up window
                time_diff = last_msg_date - after_date
                if time_diff.total_seconds() > 0 and time_diff.total_seconds() <= (window_hours * 3600):
                    return True
            except Exception as e:
                # Try fallback to ISO format
                try:
...
REMEMBER: RETURN ONLY THE JSON OBJECT, NOTHING ELSE. NO MARKDOWN, NO EXPLANATIONS."""
    ...
    def _extract_name_from_attendees(self, email: str, attendees_list: List[Dict[str, Any]]) -> str:
        """Extract contact name from calendar attendees list.

        Args:
            email: Contact email address
            attendees_list: List of attendees with 'email' and 'display_name' fields

        Returns:
            Contact name (from display_name if available, otherwise from email)
        """
        email_lower = email.lower()
    ...
    def _has_prior_contact(self, contact_email: str) -> bool:
        """Check if user has ever sent an email TO this contact.

        Args:
            contact_email: Contact's email address

        Returns:
            True if user has sent at least one email to this contact
        """
        if not self._use_supabase:
            return True  # Assume yes if we can't check

        try:
            # Query email_archive for outbound emails to this contact
            # Check if contact_email appears in to_emails where user is sender
            result = self.supabase.client.table('email_archive')\
                .select('id')\
                .eq('owner_id', self.owner_id)\
                .contains('to_emails', [contact_email])\
                .limit(1)\
                .execute()
