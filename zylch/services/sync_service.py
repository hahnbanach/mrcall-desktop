    ...
    async def sync_pipedrive(self) -> Dict[str, Any]:
        ...
        pipedrive_creds = (
            self.supabase.get_provider_credentials(
                self.owner_id, "pipedrive"
            )
        )
        if (
            not pipedrive_creds
            or not pipedrive_creds.get("api_token")
        ):
            logger.info(
                "[pipedrive_sync] Skipping"
        ...
        try:
            from zylch.tools.pipedrive import (
                PipedriveClient,
            )

            pipedrive = PipedriveClient(
                api_token=pipedrive_creds["api_token"]
            )

            deals = pipedrive.list_deals(
                status="all_not_deleted", limit=500
            )
            logger.info(
                f"[pipedrive_sync] Fetched"
                f" {len(deals)} deals"
            )
            ...
            for deal in deals:
                try:
                    ...
                    person_name = deal.get(
                        "person_name"
                    ) or (
                        deal.get("person_id") or {}
                    ).get(
                        "name", ""
                    )
                    org_name = deal.get(
                    ...
                    logger.error(
                        f"[pipedrive_sync] Failed to"
                        f" store deal"
                        f" {deal.get('id')}: {e}"
                    )

            logger.info(
                f"[pipedrive_sync] Complete:"
                f" {deals_synced} deals synced"
            )
            return {
                "success": True,
