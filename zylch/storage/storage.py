...
class Storage:
    ...
    def remove_trigger(self, owner_id: str, trigger_id: str) -> bool:
        ...
        with get_session() as session:
            session.query(Trigger).filter(
                Trigger.owner_id == owner_id, Trigger.id == trigger_id
            ).delete()
    ...
    def register_share_recipient(
        self, sender_id: str, sender_email: str, recipient_email: str
    ) -> Dict[str, Any]:
        ...
        with get_session() as session:
            ...
            logger.info(f"Registered share recipient " f"{recipient_email} for sender {sender_id}")
            return row.to_dict() if row else {}

    def revoke_sharing(self, sender_id: str, recipient_email: str) -> bool:
        """Revoke sharing access for a recipient."""
        with get_session() as session:
            session.query(SharingAuth).filter(
                SharingAuth.sender_id == sender_id,
                SharingAuth.recipient_email == recipient_email,
    ...
    def get_oauth_state(self, state: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            with get_session() as session:
                row = session.query(OAuthState).filter(OAuthState.state == state).first()
    ...
    def cleanup_expired_oauth_states(self) -> int:
        ...
        try:
            now = datetime.now(timezone.utc)
            with get_session() as session:
                deleted = session.query(OAuthState).filter(OAuthState.expires_at < now).delete()
                if deleted > 0:
    ...
    def create_sendgrid_message_mapping(
        ...
        try:
            with get_session() as session:
                stmt = sqlite_insert(SendgridMessageMapping).values(**data)
                stmt = stmt.on_conflict_do_update(
                ...
                session.execute(stmt)
                logger.debug(
                    f"Created SendGrid message mapping: {sendgrid_message_id} -> {message_id}"
                )
                return data
        except Exception as e:
    ...
    def get_sendgrid_message_mapping(
        ...
        try:
            with get_session() as session:
                row = (
                    session.query(SendgridMessageMapping)
        ...
        try:
            with get_session() as session:
                ...
                if existing:
                    ...
                    logger.debug(
                        f"Updated read event for {sendgrid_message_id}, count: {read_count}"
                    )
                else:
        ...
        try:
            with get_session() as session:
                ...
                if existing:
                    ...
                    logger.debug(
                        f"Updated read event for tracking_id {tracking_id}, count: {read_count}"
                    )

                    # Update emails.read_events
                    self._update_message_read_events(
                        message_id, recipient_email, timestamp, read_count
                    )
    ...
    def _update_message_read_events(
        ...
        try:
            with get_session() as session:
                email = session.query(Email).filter(Email.id == message_id).first()
    ...
    # -- WhatsApp storage methods --------------------------------------

    def mark_whatsapp_memory_processed(
        self,
        owner_id: str,
        message_id: str,
    ) -> None:
        """Mark a WhatsApp message as processed by memory agent."""
        with get_session() as session:
    ...
    def get_unprocessed_whatsapp_messages(
        ...
        """Get WhatsApp messages not yet processed by memory agent."""
        cutoff = datetime.now(timezone.utc) - timedelta(days=days_back)
        ...
        with get_session() as session:
            rows = (
                ...
                .filter(
                    ...
                    WhatsAppMessage.text.isnot(None),
    ...
    def get_whatsapp_message_count(self, owner_id: str) -> int:
        """Get total count of WhatsApp messages for user."""
        with get_session() as session:
            return (
    ...
    def get_whatsapp_contact_count(self, owner_id: str) -> int:
        """Get total count of WhatsApp contacts for user."""
        with get_session() as session:
            return (
